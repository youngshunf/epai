    ...
    <?= GridView::widget([
        ...
        'columns' => [
            ...
            ['attribute'=>'审核状态','value'=>function ($model){
                return CommonUtil::getDescByValue('auction_goods', 'auth_status', $model->status);
            }],
               ...
               return CommonUtil::fomatTime($model->created_at);
            	...
            	'template'=>'{view-goods}{update-goods}{delete-goods}',
	             'buttons'=>[
					...
					return  Html::a('<span class="glyphicon glyphicon-trash"></span>', $url, ['title' => '删除拍品', 'data-confirm'=>'是否确定删除该分类以及该分类下的所有资讯？'] );
					},
