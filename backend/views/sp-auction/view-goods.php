...
?>
    ...
    <?php if($model->auth_status!=1){?>
         <?= Html::a('审核通过', ['pass-goods', 'id' => $model->id], ['class' => 'btn btn-success','data-confirm'=>'您确定要审核通过此拍品吗？']) ?>
        <?php }?>
        <?php if($model->auth_status!=2){?>
         <?= Html::a('审核不通过', ['deny-goods', 'id' => $model->id], ['class' => 'btn btn-warning','data-confirm'=>'您确定要审核不通过此拍品吗?']) ?>
        <?php }?>
